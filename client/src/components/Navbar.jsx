...
function Navbar() {
  ...
  return (
    ...
    <div className='bg-black h-14 flex justify-between items-center px-16 border border-solid border-2 border-white rounded-full mx-5'>
        ...
        <Link to='/'><p className={`py-2 px-8 text-white  border-bg-amber-300 rounded-full font-semibold  bg-emerald-400 border-amber-300 `}>AlgoAid.Ai</p></Link>
        ...
        <ul className='flex  pl-10 text-sm text-white border-b-red-600'>
            ...
            <Link to='/learn'><li className={`py-2 px-8 mx-2 text-white border border-white rounded-full font-semibold ${pathMathRouter("/Donno") && "bg-amber-300 text-black border-black font-semibold"}`}>Learn</li></Link>
        </ul>
        </div>
        </div>
    </div>
